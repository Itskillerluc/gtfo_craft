...
public class EntitySmallShadow extends ModEntity implements IAnimatable {

    private final AnimationFactory factory = new AnimationFactory(this);

    private static final DataParameter<Boolean> ATTACKING =
            EntityDataManager.createKey(EntitySmallShadow.class, DataSerializers.BOOLEAN);

    public EntitySmallShadow(World worldIn) {
        super(worldIn);
        setSize(1.2f, 3.5f);
    }
    ...
    private <E extends IAnimatable> PlayState predicate(AnimationEvent<E> event) {
        ...
        if (event.isMoving() && !dataManager.get(ATTACKING)) {
            builder.addAnimation("animation.small_shadow.walk", ILoopType.EDefaultLoopTypes.LOOP);
            cont = true;
        }

        if (dataManager.get(ATTACKING)) {
            builder.addAnimation("animation.small_shadow.attack", ILoopType.EDefaultLoopTypes.PLAY_ONCE);
            cont = true;
        }
    ...
    class StrikerAttackGoal extends EntityAIAttackMelee {
        private EntitySmallShadow entity;
        private int animCounter = 0;
        private int animTickLength = 20;

        public StrikerAttackGoal(EntitySmallShadow pMob, double pSpeedModifier, boolean pFollowingTargetEvenIfNotSeen) {
