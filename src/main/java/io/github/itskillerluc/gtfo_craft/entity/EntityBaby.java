...
public class EntityBaby extends EntityMob implements IAnimatable {
    ...
    private <E extends IAnimatable> PlayState predicate(AnimationEvent<E> event) {
        ...
        if (event.isMoving() && !dataManager.get(ATTACKING)) {
            builder.addAnimation("animation.baby.walk", ILoopType.EDefaultLoopTypes.LOOP);
            cont = true;
        }

        if (dataManager.get(ATTACKING)) {
            builder.addAnimation("animation.baby.attack", ILoopType.EDefaultLoopTypes.PLAY_ONCE);
            cont = true;
        }
    ...
    class StrikerAttackGoal extends EntityAIAttackMelee {
        private EntityBaby entity;
        private int animCounter = 0;
        private int animTickLength = 20;

        public StrikerAttackGoal(EntityBaby pMob, double pSpeedModifier, boolean pFollowingTargetEvenIfNotSeen) {
