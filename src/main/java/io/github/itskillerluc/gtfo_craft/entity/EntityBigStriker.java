...
public class EntityBigStriker extends ModEntity implements IAnimatable {
    ...
    private <E extends IAnimatable> PlayState predicate(AnimationEvent<E> event) {
        ...
        if (event.isMoving() && !dataManager.get(ATTACKING)) {
            builder.addAnimation("animation.big_striker.walk", ILoopType.EDefaultLoopTypes.LOOP);
            cont = true;
        }

        if (dataManager.get(ATTACKING)) {
            builder.addAnimation("animation.big_striker.attack", ILoopType.EDefaultLoopTypes.PLAY_ONCE);
            cont = true;
        }
    ...
    class StrikerAttackGoal extends EntityAIAttackMelee {
        private EntityBigStriker entity;
        private int animCounter = 0;
        private int animTickLength = 20;

        public StrikerAttackGoal(EntityBigStriker pMob, double pSpeedModifier, boolean pFollowingTargetEvenIfNotSeen) {
