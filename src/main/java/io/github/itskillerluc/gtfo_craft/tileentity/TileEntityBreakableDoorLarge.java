...
public class TileEntityBreakableDoorLarge extends TileEntityDoor implements IAnimatable, ITickable {
    ...
    public void open() {
        ...
        for (int i = 2; i >= 0; i--) {
            for (int j = 0; j <= 4; j++) {
                ...
                if (!world.getBlockState(pos).getBlock().equals(BlockRegistry.BREAKABLE_DOOR_LARGE_CONTROLLER)) continue;
                ...
                if (world.getBlockState(blockPos).getBlock().equals(BlockRegistry.BREAKABLE_DOOR_LARGE_CONTROLLER)) continue;
                TileEntityDoorHelper.Location location;
                world.setBlockState(blockPos, world.getBlockState(blockPos).withProperty(BlockBreakableDoorLargeHelper.OPEN, true).withProperty(BlockBreakableDoorLargeHelper.POWERED, true));
    ...
    public void close() {
        ...
        for (int i = 2; i >= 0; i--) {
            for (int j = 0; j <= 4; j++) {
                ...
                if (world.getBlockState(blockPos).getBlock().equals(BlockRegistry.BREAKABLE_DOOR_LARGE_CONTROLLER)) continue;
                TileEntityDoorHelper.Location location;
                world.setBlockState(blockPos, world.getBlockState(blockPos).withProperty(BlockBreakableDoorLargeHelper.OPEN, false));
                ((TileEntityDoorHelper) world.getTileEntity(blockPos)).master = this.pos;
                if (j == 4 && i == 2) {
                    location = TileEntityDoorHelper.Location.CORNERR;
                } else if (j == 4) {
