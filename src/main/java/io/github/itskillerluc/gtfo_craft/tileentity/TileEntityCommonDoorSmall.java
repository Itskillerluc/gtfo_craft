    ...
    public void destroy() {
        ...
        for (int i = 3; i >= 0; i--) {
            for (int j = 0; j <= 3; j++) {
                ...
                if (world.getBlockState(blockPos).getBlock().equals(BlockRegistry.COMMON_DOOR_SMALL_CONTROLLER)) continue;
                TileEntityDoorHelper.Location location;
                world.setBlockState(blockPos, world.getBlockState(blockPos).withProperty(BlockCommonDoorSmallHelper.OPEN, true).withProperty(BlockCommonDoorSmallHelper.POWERED, true));
                ((TileEntityDoorHelper) world.getTileEntity(blockPos)).master = this.pos;
                if (j == 3 && i == 3) {
                    location = TileEntityDoorHelper.Location.CORNERR;
                } else if (j == 3) {
    ...
    public void close() {
        ...
        for (int i = 3; i >= 0; i--) {
            for (int j = 0; j <= 3; j++) {
                ...
                if (world.getBlockState(blockPos).getBlock().equals(BlockRegistry.COMMON_DOOR_SMALL_CONTROLLER)) continue;
                TileEntityDoorHelper.Location location;
                world.setBlockState(blockPos, world.getBlockState(blockPos).withProperty(BlockCommonDoorSmallHelper.OPEN, false));
                ((TileEntityDoorHelper) world.getTileEntity(blockPos)).master = this.pos;
                if (j == 3 && i == 3) {
                    location = TileEntityDoorHelper.Location.CORNERR;
                } else if (j == 3) {
