...
import io.github.itskillerluc.gtfo_craft.block.BlockApexDoorLargeHelper;
...
public class TileEntityApexDoorLarge extends TileEntityDoor implements IAnimatable, ITickable {
    ...
    public void openInternal() {
        ...
        for (int i = 3; i >= 0; i--) {
            for (int j = 0; j <= 6; j++) {
                ...
                if (!world.getBlockState(pos).getBlock().equals(BlockRegistry.APEX_DOOR_LARGE_CONTROLLER)) continue;
                ...
                if (world.getBlockState(blockPos).getBlock().equals(BlockRegistry.APEX_DOOR_LARGE_CONTROLLER)) continue;
                TileEntityDoorHelper.Location location;
                world.setBlockState(blockPos, world.getBlockState(blockPos).withProperty(BlockApexDoorLargeHelper.OPEN, true).withProperty(BlockApexDoorLargeHelper.POWERED, true));
