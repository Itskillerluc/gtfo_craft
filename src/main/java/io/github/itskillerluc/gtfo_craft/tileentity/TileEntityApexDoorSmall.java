...
public class TileEntityApexDoorSmall extends TileEntityDoor implements IAnimatable, ITickable {
    ...
    public void openInternal() {
        ...
        for (int i = 3; i >= 0; i--) {
            for (int j = 0; j <= 3; j++) {
                ...
                if (!world.getBlockState(pos).getBlock().equals(BlockRegistry.APEX_DOOR_SMALL_CONTROLLER)) continue;
                ...
                if (world.getBlockState(blockPos).getBlock().equals(BlockRegistry.APEX_DOOR_SMALL_CONTROLLER)) continue;
                TileEntityDoorHelper.Location location;
                world.setBlockState(blockPos, world.getBlockState(blockPos).withProperty(BlockApexDoorSmallHelper.OPEN, true).withProperty(BlockApexDoorSmallHelper.POWERED, true));
