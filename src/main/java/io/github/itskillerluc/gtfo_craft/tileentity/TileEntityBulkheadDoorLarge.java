...
public class TileEntityBulkheadDoorLarge extends TileEntityDoor implements IAnimatable, ITickable {
        ...
        for (int k = -1; k <= 1; k++) {
            for (int i = 3; i >= 0; i--) {
                for (int j = 0; j <= 6; j++) {
                    ...
                    if (!world.getBlockState(pos).getBlock().equals(BlockRegistry.BULKHEAD_DOOR_LARGE_CONTROLLER))
                        continue;
                    ...
                    if (world.getBlockState(blockPos).getBlock().equals(BlockRegistry.BULKHEAD_DOOR_LARGE_CONTROLLER))
                        continue;
                    TileEntityDoorHelper.Location location;
                    world.setBlockState(blockPos, world.getBlockState(blockPos).withProperty(BlockBulkheadDoorLargeHelper.OPEN, i < 3 && j > 1 && j < 5).withProperty(BlockBulkheadDoorLargeHelper.POWERED, true));
