...
import io.github.itskillerluc.gtfo_craft.block.BlockPossessedSecurityDoorSmallHelper;
...
public class TileEntityPossessedSecurityDoorSmall extends TileEntityDoor implements IAnimatable, ITickable {
    ...
    public void openInternal() {
        ...
        for (int i = 3; i >= 0; i--) {
            for (int j = 0; j <= 3; j++) {
                ...
                if (!world.getBlockState(pos).getBlock().equals(BlockRegistry.POSSESSED_SECURITY_DOOR_SMALL_CONTROLLER)) continue;
                ...
                if (world.getBlockState(blockPos).getBlock().equals(BlockRegistry.POSSESSED_SECURITY_DOOR_SMALL_CONTROLLER)) continue;
                TileEntityDoorHelper.Location location;
                world.setBlockState(blockPos, world.getBlockState(blockPos).withProperty(BlockPossessedSecurityDoorSmallHelper.OPEN, true).withProperty(BlockPossessedSecurityDoorSmallHelper.POWERED, true));
