...
public class BlockBulkheadDoorSmallController extends BlockDoorController implements ITileEntityProvider {
            ...
            for (int i = 3; i >= 0; i--) {
                for (int j = 0; j <= 3; j++) {
                    ...
                    if (!worldIn.getBlockState(new BlockPos(eastWest ? xz.getX() : zx.getX(), y.getY(), eastWest ? zx.getZ() : xz.getZ())).getMaterial().isReplaceable())
                        continue;
                    worldIn.setBlockState(new BlockPos(eastWest ? xz.getX() : zx.getX(), y.getY(), eastWest ? zx.getZ() : xz.getZ()), BlockRegistry.BULKHEAD_DOOR_SMALL_HELPER.getDefaultState()
        ...
        for (int k = -1; k <= 1; k++) {
            for (int i = 3; i >= 0; i--) {
                for (int j = 0; j <= 3; j++) {
                    ...
                    BlockPos y = pos.offset(EnumFacing.UP, i);
                    if (world.getBlockState(new BlockPos(eastWest ? xz.getX() : zx.getX(), y.getY(), eastWest ? zx.getZ() : xz.getZ())).getBlock().equals(BlockRegistry.BULKHEAD_DOOR_SMALL_CONTROLLER))
                        continue;
    ...
    public AxisAlignedBB getBoundingBox(IBlockState state, IBlockAccess source, BlockPos pos) {
        if (state.getValue(OPEN)) {
            EnumFacing enumfacing = state.getValue(BlockBulkheadDoorSmallHelper.FACING);
            ...
            if (!isInversed) {
                return isEastWest ? BlockBulkheadDoorSmallHelper.EAST_RIGHT : BlockBulkheadDoorSmallHelper.NORTH_LEFT;
            } else {
                return isEastWest ? BlockBulkheadDoorSmallHelper.EAST_LEFT : BlockBulkheadDoorSmallHelper.NORTH_RIGHT;
            }
        }
        ...
        switch (enumfacing) {
            case WEST:
            case EAST:
                return BlockBulkheadDoorSmallHelper.EAST_CENTER;
            default:
                return BlockBulkheadDoorSmallHelper.NORTH_CENTER;
        }
    }
    @Override
