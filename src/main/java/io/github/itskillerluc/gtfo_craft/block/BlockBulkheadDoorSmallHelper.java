...
public class BlockBulkheadDoorSmallHelper extends BlockDoorHelper implements ITileEntityProvider {
    protected static final AxisAlignedBB NORTH_CENTER = new AxisAlignedBB(0, 0.0D, 0D, 1, 1.0D, 1D);
    protected static final AxisAlignedBB EAST_CENTER = new AxisAlignedBB(0D, 0.0D, 0, 1, 1.0D, 1);
    ...
    public void breakBlock(World worldIn, BlockPos pos, IBlockState state) {
        if (worldIn.getTileEntity(pos) instanceof TileEntityDoorHelper) {
            BlockBulkheadDoorSmallController.breakDoor(worldIn, ((TileEntityDoorHelper) worldIn.getTileEntity(pos)).master, state.getValue(FACING).getOpposite());
        }
