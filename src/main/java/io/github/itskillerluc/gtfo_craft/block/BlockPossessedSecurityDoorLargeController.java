...
public class BlockPossessedSecurityDoorLargeController extends BlockDoorController implements ITileEntityProvider {
    ...
    public BlockPossessedSecurityDoorLargeController(Material blockMaterialIn, MapColor blockMapColorIn) {
        ...
        setUnlocalizedName("possessed_security_door_large");
    ...
    public void onBlockPlacedBy(World worldIn, BlockPos pos, IBlockState state, EntityLivingBase placer, ItemStack stack) {
        ...
        for (int i = 3; i >= 0; i--) {
            for (int j = 0; j <= 6; j++) {
                ...
                worldIn.setBlockState(new BlockPos(eastWest ? xz.getX() : pos.getX(), y.getY(), eastWest ? pos.getZ() : xz.getZ()), BlockRegistry.POSSESSED_SECURITY_DOOR_LARGE_HELPER.getDefaultState()
                        .withProperty(BlockPossessedSecurityDoorLargeHelper.FACING, placer.getHorizontalFacing().rotateY()), 3);
    ...
    public static void breakDoor(World world, BlockPos pos, EnumFacing facing) {
        for (int i = 3; i >= 0; i--) {
            for (int j = 0; j <= 6; j++) {
                ...
                BlockPos y = pos.offset(EnumFacing.UP, i);
                if (world.getBlockState(new BlockPos(eastWest ? xz.getX() : pos.getX(), y.getY(), eastWest ? pos.getZ() : xz.getZ())).getBlock().equals(BlockRegistry.POSSESSED_SECURITY_DOOR_LARGE_CONTROLLER)) continue;
    ...
    public AxisAlignedBB getBoundingBox(IBlockState state, IBlockAccess source, BlockPos pos) {
        if (state.getValue(OPEN)) {
            EnumFacing enumfacing = state.getValue(BlockPossessedSecurityDoorLargeHelper.FACING);
            ...
            if (!isInversed) {
                return isEastWest ? BlockPossessedSecurityDoorLargeHelper.EAST_RIGHT : BlockPossessedSecurityDoorLargeHelper.NORTH_LEFT;
            } else {
                return isEastWest ? BlockPossessedSecurityDoorLargeHelper.EAST_LEFT : BlockPossessedSecurityDoorLargeHelper.NORTH_RIGHT;
            }
        }
