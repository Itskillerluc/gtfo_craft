...
public class BlockApexDoorLargeController extends BlockDoorController implements ITileEntityProvider {

    public BlockApexDoorLargeController(Material blockMaterialIn, MapColor blockMapColorIn) {
        super(blockMaterialIn, blockMapColorIn);
        setCreativeTab(GtfoCraftCreativeTab.INSTANCE);
        setRegistryName(new ResourceLocation(GtfoCraft.MODID, "apex_door_large_controller"));
        setUnlocalizedName("apex_door_large");
    ...
    public TileEntity createNewTileEntity(World worldIn, int meta) {
        return new TileEntityApexDoorLarge();
    }
    ...
    public void onBlockPlacedBy(World worldIn, BlockPos pos, IBlockState state, EntityLivingBase placer, ItemStack stack) {
        ...
        for (int i = 3; i >= 0; i--) {
            for (int j = 0; j <= 6; j++) {
                ...
                worldIn.setBlockState(new BlockPos(eastWest ? xz.getX() : pos.getX(), y.getY(), eastWest ? pos.getZ() : xz.getZ()), BlockRegistry.APEX_DOOR_LARGE_HELPER.getDefaultState()
                        .withProperty(BlockApexDoorLargeHelper.FACING, placer.getHorizontalFacing().rotateY()), 3);
    ...
    public static void breakDoor(World world, BlockPos pos, EnumFacing facing) {
        for (int i = 3; i >= 0; i--) {
            for (int j = 0; j <= 6; j++) {
                ...
                BlockPos y = pos.offset(EnumFacing.UP, i);
                if (world.getBlockState(new BlockPos(eastWest ? xz.getX() : pos.getX(), y.getY(), eastWest ? pos.getZ() : xz.getZ())).getBlock().equals(BlockRegistry.APEX_DOOR_LARGE_CONTROLLER)) continue;
    ...
    public AxisAlignedBB getBoundingBox(IBlockState state, IBlockAccess source, BlockPos pos) {
        if (state.getValue(OPEN)) {
            EnumFacing enumfacing = state.getValue(BlockApexDoorLargeHelper.FACING);
            ...
            if (!isInversed) {
                return isEastWest ? BlockApexDoorLargeHelper.EAST_RIGHT : BlockApexDoorLargeHelper.NORTH_LEFT;
            } else {
                return isEastWest ? BlockApexDoorLargeHelper.EAST_LEFT : BlockApexDoorLargeHelper.NORTH_RIGHT;
            }
        }
        ...
        switch (enumfacing) {
            case WEST:
            case EAST:
                return BlockApexDoorLargeHelper.EAST_CENTER;
            default:
                return BlockApexDoorLargeHelper.NORTH_CENTER;
        }
    }
    @Override
