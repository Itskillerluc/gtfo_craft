...
public class BlockBulkheadDoorLargeHelper extends Block implements ITileEntityProvider {
        ...
        if (blockIn != this && (flag || blockIn.getDefaultState().canProvidePower()) && flag != iblockstate1.getValue(POWERED))
        {
            ...
            if (flag != state.getValue(OPEN))
            {
                if (worldIn.getTileEntity(pos) instanceof TileEntityBulkheadDoorHelper && worldIn.getTileEntity(pos) != null) {
                    ((TileEntityBulkheadDoorLarge) worldIn.getTileEntity(((TileEntityBulkheadDoorHelper) worldIn.getTileEntity(pos)).master)).open();
